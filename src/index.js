...
if (url.pathname === "/api/day") {
  ...
  return json(await getDayDetails(env, day));
}
if (url.pathname === "/api/ip") {
  const ip = url.searchParams.get("ip");
  return json(await getIPDetails(env, ip));
}
if (url.pathname === "/api/domain") {
  const domain = url.searchParams.get("domain");
  return json(await getDomainDetails(env, domain));
}
...
if (url.pathname === "/api/ai_explain") {
  ...
  return json({ text: await explainWithAI(env, data) });
}
...
if (r.length) dkimRecords.push(...r.map((x) => x.data));
}
...
const issues = [];
if (!records.spf.length) issues.push("No SPF record configured");
if (records.spf.length > 1) issues.push("Multiple SPF records detected");
if (!records.dmarc.length) issues.push("DMARC record missing");
if (records.dmarc[0]?.includes("p=none")) issues.push("DMARC policy not enforced");
if (!records.dkim.length) issues.push("No DKIM detected");
...
const r = await env.DB.prepare(`     SELECT source_ip,SUM(count) failures
    FROM dmarc_records
...
const r = await env.DB.prepare(`     SELECT source_ip,domain,SUM(count) total,
    SUM(CASE WHEN disposition!='none' THEN count ELSE 0 END) failures
    FROM dmarc_records
...
// ---------- AI ----------
async function explainWithAI(env, data) {
...
role: "system",
content:
"Explain DMARC analytics and potential spoofing issues in simple terms.",
},
...
.header{background:white;border-bottom:3px solid #e30613;padding:15px;display:flex;gap:10px}
.logo{height:36px}
...
.alert{background:#ffe8e8;border-left:5px solid #e30613;padding:10px;margin-bottom:20px;display:none}
